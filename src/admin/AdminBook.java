...
public class AdminBook {
	 ...
	 * 
	 * @author sojin Jang
	 * 관리자의 도서관리의 "도서목록 조회 클래스"
	 */
	public class SelectBookList {

		/**
		 * 십진 대분류 리스트의 페이지를 위한 메소드
		 * @param arrayList 십진 대분류 리스트를 담는 객체
		...
		public void SelectBook(AdminUser adminUser) {
			...
			try {
				...
				if (rs.next()) {
				while (rs.next()) {
					list.add(rs.getString("seq")+"\t\t"+rs.getString("도서번호")+"\t"+rs.getString("분류")+"\t"+rs.getString("위치")+"\t"+ rs.getString("도서명"));
	 ...
	 * 
	 * @author sojin Jang
	 * 관리자의 도서관리의 "대여한 도서의 목록 확인과 신규 대여 도서 추가 가능 클래스" 
	 *
	 */
...
//						break;
		...
		/**
		 *  현재 소장한 도서 목록을 조회할 수 있는 메소드
		 *  - 조회를 원하는 도서코드를 입력하면 관련된 도서의 목록 확인 가능
		 * @param adminUser 로그인한 관리자 객체
		 */
		public void SelectBook(AdminUser adminUser) {
		...
		/**
		 * 신규 대여 도서의 목록을 추가하기 위한 메소드
		 * - 회원번호와 도서의 코드를 입력하여 저장 가능
		 * @param adminUser 로그인한 관리자의 객체
		...
		/**
		 * 당일 날짜의 대여 목록을 모두 조회 할 수 있는 메소드
		 * - 당일 날짜의 회원번호로 대여한 모든 도서의 정보 확인 가능
	...
	//3. 예약확인
		...
		/**
		 * 예약 도서 목록을 날짜별로 조회할 수 있는 클래스
		 * 예약한 회원정보와 도서정보 확인 가능
		 * @param adminUser 로그인한 관리자의 객체
		 */
	 ...
	 * 
	 * @author sojin Jang
	 * 관리자 도서관리의 "반납한 도서의 목록 조회와 새로 추가할수 있는 클래스"
	 *
	 */
	public class ReturnBook {
		...
		/**
		 * 도서 반납 처리하기 전 도서의 목록을 출력할 수 있는 메소드
		 * - 도서코드로 확인 가능
		 * @param adminUser 로그인한 관리자의 객체
		 */
		...
		public void returnstate(String InputBookCode, AdminUser adminUser) {
			...
			if (MemberNumInput.equals("0")) {
				...
				try {
					...
					// sql select 명령
					String selectsql = String.format("select * vwCheckReturnBook where 도서코드 = %s and 회원번호 = %s",
		...
		public void CheckPeriodBook(AdminUser adminUser) {
			...
			try {
				...
				// 0번 입력 시
				if (SelectYearInput.equals("0") || SelectMonthInput.equals("0") || SelectDayInput.equals("0")
						|| LastYearInput.equals("0") || LastMonthInput.equals("0") || LastDayInput.equals("0")) {
					// 뒤로가기
					return;

				} else if (Pattern.matches("^[0-9]*$", SelectYearInput) || Pattern.matches("^[0-9]*$", SelectMonthInput)
						|| Pattern.matches("^[0-9]*$", SelectDayInput) || Pattern.matches("^[0-9]*$", LastYearInput)
						|| Pattern.matches("^[0-9]*$", LastMonthInput) || Pattern.matches("^[0-9]*$", LastDayInput)) {
					...
					// sql select 명령
					String sql = String.format("select * from vwCheckReturnBook where 반납날짜 between '%s' and '%s'",
				...
				System.out.println("\n\t\t\t〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓");
				System.out.printf("\n\t\t\t[%s년%s월%s일]부터 [%s년%s월%s일]까지의 반납일 리스트 입니다.\n", SelectYearInput, SelectMonthInput,
						SelectDayInput, LastYearInput, LastMonthInput, LastDayInput);
				System.out.println(
						"\t\t\t〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓");
				System.out.println("\t\t\t[번호]\t[도서코드]\t\t[고객명]\t[회원번호]\t[대여일]\t[반납일]\t[반납여부]\t[연장횟수]\t\t[도서명]");
				System.out.println(
						"\t\t\t〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓");
				...
				if (rs.next()) {
					...
					while (rs.next()) {
						list.add(rs.getString("도서코드") + "\t" + rs.getString("성함") + "\t\t" + rs.getString("회원번호") + "\t\t"
								+ rs.getString("대여날짜") + "\t" + rs.getString("반납날짜") + "\t" + rs.getString("반납여부") + "\t\t"
...
//			// 0번 입력 시 뒤로가기
//			if (DecimalInput.equals("0") || MiddleSeriesInput.equals("0") || SmallSeriesInput.equals("0")
//					|| location.equals("0") || BookNameInput.equals("0") || PublisherInput.equals("0")
//					|| AuthorInput.equals("0")) {
//				// 뒤로가기
//				UpdateBookList(adminUser);
	//
//			} else {
										...
										try {
											...
											// 도서 코드 생성
											String Bookcode = DecimalInput + "-" + BookNameInput.substring(0, 1)
													+ AuthorInput.substring(0, 1) + "-s" + MiddleSeriesInput + "-" + "v0"
													+ SameBook;

											// 유효성검사
											if (DecimalInput.length() < 1000 && MiddleSeriesInput.length() < 100
													&& SmallSeriesInput.length() < 10) {
											...
											System.out.printf(
													"\n\t\t\t[도서명]: %s\n\t\t\t[출판사]: %s\n\t\t\t[저자]: %s\n\t\t\t[대분류 코드]: %s\n\t\t\t[중분류 코드]: %s\n\t\t\t[소분류 코드]: %s\n\t\t\t[도서코드]: %s\n\t\t\t[중복책권수]: %s\n\t\t\t[위치]: %s\n",
													BookNameInput, PublisherInput, AuthorInput, DecimalInput,
													MiddleSeriesInput, SmallSeriesInput, Bookcode, SameBook, location);
	 ...
	 * @param 로그인한 관리자의 객체
	 */
		public void DecimalMiddle(AdminUser adminUser) {
			// 도서 십진분류코드 중분류
			...
			try {
				...
				String middlesql = "{ call ProcDecimalMiddle(?) }";

				// open
				conn = util.open();
				stat = conn.prepareCall(middlesql);
		 ...
		 * @param 로그인한 관리자의 객체
		 */
		public void DecimalList(AdminUser adminUser) {
			// 도서 십진분류코드 대분류 리스트
			...
			try {
				...
				// 프로시저 부르기
				String bicsql = "{ call ProcDecimalBig(?) }";

				// open
				conn = util.open();
				stat = conn.prepareCall(bicsql);
