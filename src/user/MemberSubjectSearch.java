 ...
 *
 */
public class MemberSubjectSearch {
	List<String[]> notReturnList;//반납하지 않은 책 정보
	List<String[]> bookList;//해당 제목의 책 정보를 넣어주기 위함.
	...
	public void search(MemberUser user) {
		...
		String inputName = scan.nextLine();//책 제목을 입력받는다.
		System.out.println("\t\t\t〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓");
		...
		bookList = new ArrayList<String[]>();//해당 책 제목의 정보를 넣어주기 위함. **새로운 객체 생성

		try {
			
			
			String sql = "{call procSubjectSearch(?,?)}";//책 제목으로 책을 찾아주는 프로시저
			
			conn = util.open("localhost", "lms", "java1234");
			stat = conn.prepareCall(sql);
			
			
			stat.setString(1, inputName);//책제목 넣어주기
...
//						System.out.println("왜 미반납으로 뜨는거야");
		...
		if (bookList.size() != 0) {//해당 도서의 정보가 존재하는 경우 -> 대여가능한지 예약으로 빠져야하는지가 다음단계
			...
			printBookList = spf.searchSubject(inputName, bookList);//중복되는 책이름을 제외한 -> 유저에게 보여줄것.
			...
			while(whileFlag) {
				System.out.println("\t\t\t〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓");
				System.out.println("\t\t\t해당 책제목을 가진 책이 존재하지 않습니다.");
